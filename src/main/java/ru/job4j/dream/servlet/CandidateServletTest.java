...
public class CandidateServletTest {
    ...
    public void whenCreatePost() throws IOException {
        ...
        PowerMockito.when(req.getParameter("skills")).thenReturn("s");
        PowerMockito.when(req.getParameter("position")).thenReturn("p");

        new CandidateServlet().doPost(req, resp);

        Candidate result = store.findAllCandidates().iterator().next();
        Assert.assertThat(result.getName(), Is.is("n"));
        Assert.assertThat(result.getSkills(), Is.is("s"));
        Assert.assertThat(result.getPosition(), Is.is("p"));
    }
}
